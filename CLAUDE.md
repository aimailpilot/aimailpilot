...
> - `lead-intelligence-engine.ts` is self-contained (own Gmail+Outlook token helpers, own classifier). Two protected invariants: (1) All SELECT aliases in raw SQL queries MUST be double-quoted (e.g. `as "contactEmail"`, `as "totalReceived"`) — PostgreSQL lowercases unquoted aliases causing JS to read `undefined` from result rows, yielding 0 contacts analyzed (fixed 2026-04-21). (2) `buildContactSummaries` and `classifyContactsWithAI` internal helpers must keep their current signatures since `analyzeOrgLeads` and `analyzeOrgLeadsIncremental` both depend on them. Adding NEW exported entry points (e.g. `analyzeOrgLeadsIncremental` added 2026-04-28) is allowed when behavior change is needed; just don't modify the existing exported functions' contracts. Incremental mode is the default for `/api/lead-intelligence/analyze` — pass `force: true` in body for delete-and-rebuild behavior (use after AI prompt changes).
