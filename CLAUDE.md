...
> **PARKED features (code shipped, awaiting external dependency)**
> - **AI Lead Agent** (status.md #106) — Phase 1-4 complete and deployed. Search button currently surfaces `400 invalid_request_error: credit balance too low` because the `claude_api_key` configured in Advanced Settings has no balance on the Anthropic API console (separate product from the user's claude.ai Max subscription). Two unblock paths: (a) top up Anthropic credits at console.anthropic.com, OR (b) integrate a third-party search API (Tavily/Brave) so Azure OpenAI can serve as the LLM for funded/cxo/academics modes. Do NOT delete or rip out the abstraction layers (`server/lib/llm/*`, `server/services/lead-agent.ts`, `server/lib/lead-agent-*`, `client/src/pages/lead-agent.tsx`, route handlers, tests) — they are correct and tested; the gating is purely external billing.
...
> - `lead-intelligence-engine.ts` is self-contained (own Gmail+Outlook token helpers, own classifier). Three protected invariants: (1) All SELECT aliases in raw SQL queries MUST be double-quoted (e.g. `as "contactEmail"`, `as "totalReceived"`) — PostgreSQL lowercases unquoted aliases causing JS to read `undefined` from result rows, yielding 0 contacts analyzed (fixed 2026-04-21). (2) `buildContactSummaries` and `classifyContactsWithAI` internal helpers must keep their current behavior since `analyzeOrgLeads` and `analyzeOrgLeadsIncremental` both depend on them. (3) `email_accounts.leadIntelLastScanAt` is updated only on a clean scan (zero errors) — do not update on partial failures or the next attempt will skip newly-arrived emails it should have re-fetched. Adding NEW exported entry points (e.g. `analyzeOrgLeadsIncremental` added 2026-04-28) is allowed when behavior change is needed; just don't modify the existing exported functions' contracts. Both `/api/lead-intelligence/scan` and `/api/lead-intelligence/analyze` default to incremental — pass `force: true` in body for full rescan / delete-and-rebuild (use after extending monthsBack or after AI prompt changes).
...
> - No changes to `checkSendingWindow`, `getUserLocalTime`, `msUntilNextSendWindow` in campaign-engine.ts — **overnight window support was intentionally added 2026-04-24**: when `startTime > endTime` (e.g. 17:00–07:00 IST sending to USA), `checkSendingWindow` uses OR logic (`currentTime >= startTime || currentTime < endTime`) to detect in-window; gap period (e.g. 07:00–17:00) is blocked and returns `pauseUntilMs` to today's startTime. `msUntilNextSendWindow` has a matching same-day overnight branch (detects gap via `currentTime >= endTime && currentTime < startTime`). Do NOT remove: (1) the `isOvernightWindow` detection gate, (2) the OR logic for in-window check, (3) the gap detection block in `msUntilNextSendWindow`.
...
> - No changes to the deleted-account detection block in `executeFollowup()` in `followup-engine.ts` — after atomic daily-limit reservation returns no row, must call `storage.getEmailAccount(campaign.emailAccountId)`; if null, mark execution `failed` and return (do NOT defer back to `pending` — causes infinite loop). Only set `pending` when account exists but is at its daily limit. Reverting causes hundreds of executions to loop forever (caused 352 stuck executions 2026-04-25).
> - No changes to `deleteEmailAccount()` in `pg-storage.ts` — MUST be a soft-delete (`UPDATE SET "isActive" = 0`) NOT a hard DELETE. Hard DELETE removes the UUID from DB and causes orphaned campaigns/follow-ups. Soft-delete preserves the row as a foreign-key anchor so campaigns can be recovered.
> - No changes to `getEmailAccounts()` in `pg-storage.ts` — MUST filter `AND "isActive" != 0` to hide soft-deleted accounts from the UI. MUST also filter `AND COALESCE("scanOnly", 0) = 0` to hide read-only Lead Intelligence accounts from send paths, reply trackers, and the Email Accounts UI. Do NOT remove either filter. Lead Intelligence callers must use `getEmailAccountsForLeadIntel(orgId)` (added 2026-04-28) which omits the scan-only filter.
> - No changes to `getEmailAccountIncludingInactive()` in `pg-storage.ts` — uses `LOWER(email) = LOWER($2)` and includes inactive rows. This is the ONLY method that should be called in OAuth callbacks (Gmail add_sender ~L688, Outlook add_sender ~L1219, Gmail login ~L847 in `routes.ts`). Do NOT revert to `getEmailAccounts().find(a => a.email === email)` — case-sensitive JS comparison misses existing accounts when Google OAuth returns email in different case, causing a new UUID to be created on reconnect and orphaning all campaigns.
> - No changes to `"isActive" = true` in `toolCheckSenderQuota()` in `campaign-planner-agent.ts` — PostgreSQL boolean check requires `= true` not `= 1`. Reverting to `= 1` returns 0 rows from email_accounts and breaks Campaign Planner Agent (causes "Agent did not return a valid plan" errors).
> - No changes to `loadResolvedSettings()` in `server/lib/llm/index.ts` — three-tier fallback chain (caller org → superadmin org → env vars CLAUDE_API_KEY / AZURE_OPENAI_*) MUST stay. Existing services `campaign-planner-agent.ts` and `campaign-review-agent.ts` already rely on the same chain (they read `superSettings = storage.getApiSettings('superadmin')` directly); reverting to org-only lookup means platform-wide credentials in superadmin org become invisible to runLlm callers. Routing prefs (`ai_provider`, `ai_provider_<feature>`) follow per-org by design — only credential keys fall back. Do NOT swap the order or skip the env fallback.
> - No changes to retry+schema-strip in `server/lib/llm/providers/anthropic.ts` — `classifyAnthropicError` decides per call: same-request retry up to 3 attempts with exponential backoff for `overloaded_error` / 503 / 529 / 429; **schema-strip fallback specifically for `Grammar compilation is temporarily unavailable`** (drops `output_config.format` after 1 retry, retries once with system-prompt JSON instruction). `AbortError` always fatal — never retry on user cancel. Without the schema-strip fallback, every Anthropic grammar-compiler outage = total Lead Agent failure (root cause of 2026-04-30 incident, fixed in commit a1604f3).
> - No changes to streaming + 6-min hard timeout in `anthropic.ts` — `useStreaming = !!request.webSearch || maxTokens > 16000`. Without streaming, web-search calls (typical 30-120s, outliers 4-5 min) hit the SDK HTTP transport timeout when the connection sits idle between Claude's sequential searches. The 6-min `HARD_TIMEOUT_MS` is wrapped in a `Promise.race` so a hung socket can't hold the request thread for 10+ min. Do NOT lower below 4 min (genuine outliers fail) or above 8 min (defeats the purpose).
> - No changes to AbortSignal threading in `lead-agent.ts` / `runLlm` / `callAnthropic` — `runOneSearch` accepts `abortSignal`, plumbs through to `runLlm({abortSignal})` then to SDK's second-arg `{signal}` option. Cancel route in `routes.ts` flips `status='cancelled'` synchronously AND triggers `controller.abort()` from the in-process `leadAgentAbortControllers` Map. Without immediate status flip, the per-org "one job at a time" conflict check (`findActiveLeadAgentJob`) blocks new searches until the old call finishes naturally — making the Cancel button cosmetic. Without abort propagation, cancelled jobs eat Anthropic credits to completion. Both are required.
> - No changes to lead-agent runtime constants — `thinking: false`, `maxTokens: 8000` for the lead-agent runLlm call. Adaptive thinking adds 30-120s on top of web search and isn't earning its keep here (the model already gets reasoning context from search results). 8K easily fits 25 leads × ~250 tokens. Setting either to `true`/`16000` re-introduces the 240s timeout failures from the 2026-04-30 incident.
> - No changes to Lead Agent mode → provider routing — `funded`/`cxo_changes`/`academics` MUST hardcode `forceProvider: 'anthropic'` (in `lead-agent.ts runOneSearch` via `needsWebSearch`). Web search is Anthropic-only in this codebase; routing to Azure OpenAI with webSearch=true throws `LlmCapabilityError` BEFORE the API call. To enable Azure for these modes, integrate a third-party search API (Tavily/Brave) as an external pre-step — do not just remove the forceProvider line.
> - No changes to lead-agent background-job pattern in `routes.ts` — `lead_agent_job_<id>` rows in `api_settings`, 30-min TTL, 20s heartbeat via `setInterval` during the in-flight call (cleared in `try/finally` on completion or crash). TOCTOU race guard in `startLeadAgentJob` matches `lead_intel_job_*` pattern. Stale-jobs sweeper (`stale-jobs-sweeper.ts`) MUST keep `lead_agent_job_` in `JOB_TYPES` array — removing it leaks `running` rows after server crashes. The `setImmediate` runner's catch block guards `if (j.status === 'running')` before flipping to `failed` — required so a user-cancelled job (status already `cancelled`) doesn't get overwritten.
> - No changes to `resolveAutopilotConfig()` calls in `/api/campaigns/:id/send` and `/schedule` — both endpoints MUST run user-supplied autopilot through the resolver (in `server/lib/autopilot-defaults.ts`). The resolver returns the user's config when it has window-enforcement intent, else the safe default (Mon-Fri 09:00-18:00, Sat/Sun off, 1000/day, 5min delay). Without this, callers passing `autopilot: null` get NO window enforcement and emails fly out 24/7 (root cause of 2026-04-30 "campaigns sending at 23:57 IST" incident). The resolver is pure and unit-tested in `tests/unit/autopilot-defaults.test.ts`.
> - No changes to autopilot dialog persistence in `client/src/pages/campaign-detail.tsx` and `campaign-creator.tsx` — `saveUpdate` MUST always write the dialog's autopilot state into `sendingConfig` (was: silent fallback to existing config when `autopilot.enabled === false`, which discarded user edits when the dialog was closed via Cancel/X without clicking Apply). Apply button MUST NOT force-set `enabled = true` — it just closes the dialog with whatever the master toggle currently holds. Reset to defaults button MUST set Mon-Fri 9-18 / 1000/day / 5min explicitly. Removing any of these re-introduces the silent-discard UX bug.
> - No changes to `refreshSendingConfigIfStale()` in `campaign-engine.ts sendBatched()` — re-reads campaign from DB at most once per 60s during the send loop, so dialog updates take effect mid-send. **Do NOT add a sticky-flag DB-status halt path back into this helper** — the original draft did, and any internal pause-resume cycle (window-end, daily limit) caused the next iteration to halt because the flag was never reset. Manual UI pauses already work via `campaignEngine.pauseCampaign()` setting in-memory `tracker.paused = true`; SQL-level external pauses still need a server restart. This is intentional after commit 240c05b.
...
> - No changes to Reply tab filter in `pg-storage.ts` (`getInboxMessagesEnhanced`, `getInboxMessageCountEnhanced`, `getInboxStats`) — must include all THREE reply sources: (1) `"repliedBy" IS NOT NULL` (in-app replies + outbound-reply-sweeper native detections), (2) `(status = 'replied' AND "repliedAt" IS NOT NULL)` (in-app explicit reply action), (3) thread-fallback `EXISTS (SELECT 1 FROM unified_inbox ui2 WHERE ... fromEmail IN orgEmails)` (subsequent same-thread reply from an org account). Dropping the `repliedBy` clause hides outbound-reply-sweeper native detections; dropping the thread-fallback hides cases where the user's own reply was ingested as its own inbox row.
...
> - No changes to `not_replied` filter in `pg-storage.ts` (`getInboxMessagesEnhanced`, `getInboxMessageCountEnhanced`, `getInboxStats`) — must include ALL of: `"replyType" IN ('positive','negative','general')`, `status != 'replied'`, `"repliedAt" IS NULL`, `"repliedBy" IS NULL`, AND a NOT EXISTS thread-fallback that excludes threads with a subsequent message from an org account. The `repliedBy IS NULL` clause is mandatory — without it, outbound-reply-sweeper detections silently leak back into the Need Reply tab. The thread-fallback covers cases where the user's reply was ingested as its own inbox row.
> - The `outbound-reply-sweeper.ts` populates BOTH `"repliedBy"` (who replied) AND `"repliedAt"` (the native reply's `sentDateTime` from Gmail/Outlook thread API). Setting both keeps native and in-app replies indistinguishable to filter logic — Replied filter sees them, Need Reply filter excludes them, Team Scorecard counts them. The UPDATE is idempotent (`WHERE "repliedBy" IS NULL`) so a row claimed by an in-app reply flow is never overwritten. Do NOT change the sweeper to set only one of the two columns — that re-introduces the gap where natively-replied messages stay stuck in Need Reply.
...
> - No changes to `resumeDailyLimitPausedCampaigns()` in `server/index.ts` — called (1) immediately after `resetDailySentAll()` at UTC midnight, and (2) on a 15-minute `setInterval`. Finds `status='paused' AND autoPaused=true` campaigns with an org email account having `dailySent < dailyLimit`, then calls `startCampaign()` with 2s stagger. Do NOT remove either call site, change the interval below 5 minutes without load evaluation, or remove the stagger (OOM risk). Do NOT modify the `autoPaused=true` filter — user-paused campaigns (`autoPaused=false`) must never be auto-resumed by this function.
> - No changes to the overdue scheduled campaign boot block in `server/index.ts` (15s after boot) — queries `status='scheduled' AND scheduledAt <= now` and calls `startCampaign()` for each with 2s stagger. Do NOT remove the stagger or the `scheduledAt <= now` guard — without the guard, future-scheduled campaigns would fire immediately on every boot.
> - No changes to `getCampaignMessageStats()` in `pg-storage.ts` — must return `step0Sent` and `step0Bounced` fields (step-0 messages only). All three auto-heal locations in `routes.ts` use these fields to compute `totalRecipients` as `max(step0Sent + step0Bounced, contactIds.length)`. Do NOT revert to `stats.sent + stats.bounced` (all-steps sum) — inflates `totalRecipients` by counting follow-up sends.
> - No changes to `step0Count` query in `checkFollowupCompletion()` in `followup-engine.ts` — MUST filter `AND status = 'sent'`. Bounced step-0 messages must be excluded: `processCampaignFollowups()` never creates executions for bounced contacts, so counting them inflates `totalExpected` to an unreachable number, permanently stranding campaigns in `following_up`. Removing this filter caused 13 campaigns to be permanently stuck.
