...
| 71 | Follow-up Open + Click Tracking (Steps 2/3/4) — FIXED 2026-04-18 | `followup-engine.ts` `executeFollowup()` — `addTrackingPixel()` + `addClickTracking()` + `getBaseUrl()` helpers added to `FollowupEngine` class. `followupTrackingId` generated before send, injected into HTML, stored in message record and tracking event. Step 1 tracking unchanged (in `campaign-engine.ts`). Do NOT remove these helpers or the tracking injection before `sendEmail`. |
