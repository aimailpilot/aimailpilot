...
- **Gmail warmup filter** — `ownEmailsSet` in `gmail-reply-tracker.ts` must be built from both `email_accounts` AND `warmup_accounts`. Skip check uses regex-extracted email from `From` header (`fromEmail` variable). Do NOT rename back to `senderEmail` (duplicate declaration compile error).
